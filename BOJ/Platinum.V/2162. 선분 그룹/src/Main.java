...
public class Main {
    ...
    private static void checkLines(final int N, final Point[] points) {
        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                Point p1 = points[i];
                Point p2 = points[j];
                ...
                if (res == INF + 1) {
                    ...
                    if ((p1.x1 > p2.x1 && p1.x1 > p2.x2 && p1.x2 > p2.x1 && p1.x2 > p2.x2)
                            || (p2.x1 > p1.x2 && p2.x2 > p1.x1 && p2.x2 > p1.x2)) {
                        continue;
                    }
    ...
    public static double intersectionPointX(final Point p1, final Point p2) {
        if ((p1.x1 - p1.x2) * (p2.y1 - p2.y2) - (p1.y1 - p1.y2) * (p2.x1 - p2.x2) == 0) return INF + 1;
    ...
    // 교점의 Y좌표
    public static double intersectionPointY(final Point p1, final Point p2) {
        if ((p1.x1 - p1.x2) * (p2.y1 - p2.y2) - (p1.y1 - p1.y2) * (p2.x1 - p2.x2) == 0) return INF+1;
